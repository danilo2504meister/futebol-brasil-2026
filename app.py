...
if pagina == "🏠 Home":
    ...
    with col2:
        card("Maior Média de gols por jogo", f"{' | '.join(mg['CLUBE'])} - {mg.iloc[0]['MG']} gols/jogo", "📈 Média de Gols", escudo=escudo_time(mg.iloc[0]["CLUBE"]))
        card("Mais Vitórias", f"{' | '.join(vit['CLUBE'])} - {int(max_v)} vitórias", "🏆 Vitórias", escudo=escudo_time(vit.iloc[0]["CLUBE"]))
        card("Menor Média de Gols Levados", f"{' | '.join(md['CLUBE'])} - {md.iloc[0]['MD']} gols/jogo", "🛡️ Média de Gols Levados", escudo=escudo_time(md.iloc[0]["CLUBE"]))
        card("Melhor Aproveitamento de Pontos", f"{' | '.join(apr['CLUBE'])} - {apr.iloc[0]['APROVEITAMENTO']}%", "📊 Aproveitamento", escudo=escudo_time(apr.iloc[0]["CLUBE"]))
