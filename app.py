...
if pagina == "🏠 Home":
    ...
    with col1:
        ...
        card("Maior Invencibilidade Atual", f"{' | '.join(inv_home['CLUBE'])} - {int(max_inv)} jogos", escudo=escudo_time(inv_home.iloc[0]["CLUBE"]))
    ...
    with col2:
        ...
        card("Menor Média de Gols Levados", f"{md.iloc[0]['CLUBE']} - {md.iloc[0]['MD']:.2f} gols/jogo", escudo=escudo_time(md.iloc[0]["CLUBE"]))
